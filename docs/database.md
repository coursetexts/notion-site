...
- **`learning_paths`**: catalog or `visibility in (public, collaborative)` is readable; owner mutates metadata and community/research `data` (outline). Signed-in users may patch `data` on catalog courses, or on a path they were invited to (`053`). Collaborative visibility alone does not grant outline edits to casual visitors. Invitees are stored on `learning_path_invites`.
- **`learning_path_invites`**: owner insert/select/delete on their owned path (any visibility after `053`); invitees can read their own row; anyone who can read the path can list invites after `054` (people sidebar).
