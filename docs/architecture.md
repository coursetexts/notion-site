...
On Paths, Explore / Create / Community destinations use `lib/paths-routes.ts` (`/paths/all-courses`, `/paths/learning-path/new`, `/paths/community`, `/paths/profile`, etc.). The **About** nav label links to `/about` (dropdown still lists manifesto / professors / blog / support). The mobile menu shows a single Your Profile link when signed in (no nested profile-tab list).
...
| `/about` | Live | Static | About landing, product links, inaugural essay, hover term definitions |
