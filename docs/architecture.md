...
| Profile / social           | `/profile`, `/profile/{userId}`, `/users`                        | profiles, follows, interests, personal links, owned/saved paths, profile Updates (in Activity). Sidebar: public **bio**; interest chips; then compact **Currently learning** / **Previously learned** (`profiles.learning_now` / `learning_learned`) with a text-style link count under that block (opens personal links). Edit (pencil): bio + learning fields, remove interests, **+ New link** and **Save profile** on one row. Header pin nav (saved courses / paths) also shows on `/profile`. Hover a pin-nav or Learning card for resume (description, **N of M**, next topic, **Continue →**). Tabs (own): **Learning** → **Knowledge** → **Notes** → **Bookmarks** → \| → **Feed** → **Notifications**. Public omits Notes and Notifications. **Feed**: compose/list own Updates plus followed social activity (incl. followed Updates with replies). **Notifications**: followed you, liked your update, replied to you, path invites, resource submit/accept, join requests. Learning pills: **Courses** · **Learning paths** · **By you** · **Committed**. Shared SEARCH width on those tabs. |
...
**Feed** uses pills (**Following** · **Yours**) plus the shared SEARCH field. Following includes the Updates composer and your posts (likes + threaded replies), then followed social activity (including followed Updates). Cards are content-first: subject at the top, then a left thread line through the actor avatar to the name. **Notifications** (own profile only) lists: **followed you**, **liked your update**, **replied to you**, **invited you to their learning path**, **submitted a resource to your path — review it**, **accepted your resource submission**, plus join requests on your private paths. Opening the tab marks reply notifications read. Join requests also appear as a banner at the top of your profile. Reply unread still drives the header badge.

## Profile Updates

Short posts live on `profile_updates` (migration `050`) and are composed from the profile **Feed** (not a separate Updates tab). Likes use `votes.target_type = 'profile_update'`; replies use polymorphic `comments` with the same target type. Followed people’s Updates also appear under Following.

## Profile sidebar (public)

Edit on `/profile` (pencil): bio, **Currently learning**, **Previously learned**, personal links, and remove existing interest chips (no “+ New interest” in the sidebar). Bio and learning fields live on `profiles` (migration `049`) so they are readable on `/profile/{userId}`. Learning lines are compact metadata (`topic · topic · +N`); click expands. Labels, topics, and the personal-links control share one muted color. Personal links sit under **Previously learned** as a text-style count that opens a popover (not a pill after the bio). In edit mode, **+ New link** and **Save profile** share one row. Apply `049_profile_learning_summary.sql` on existing DBs. Profile **Updates** (posts, likes, threaded comments) need `050_profile_updates.sql`.
