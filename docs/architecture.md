...
| Profile / social           | `/profile`, `/profile/{userId}`, `/users`                        | profiles, follows, interests, personal links, owned/saved paths, profile Updates. Sidebar: public **bio**; interest chips; then compact **Currently learning** / **Previously learned** (`profiles.learning_now` / `learning_learned`) with a text-style link count under that block (opens personal links). Edit (pencil): bio + learning fields, remove interests, **+ New link** and **Save profile** on one row. Header pin nav (saved courses / paths) also shows on `/profile`. Hover a pin-nav or Learning card for resume (description, **N of M**, next topic, **Continue →**). Tabs: **Learning** → **Knowledge** → **Notes** (owner only) → **Activity** → **Bookmarks** → **Updates**. Learning pills: **Courses** · **Learning paths** · **By you** · **Committed**; card tags: streak → % complete → Commit/Notify → Continue on hover. Activity: **Feed** · **Your activity**; feed includes followed Updates with discussion-style replies. Updates tab: posts with likes + threaded comments (needs `050`). Shared SEARCH width on those tabs. On mobile (≤860px), tab actions align with search; edit sits beside the display name. |
...
**Activity** uses the same pill style as Learning (**Feed** · **Your activity**) plus the shared SEARCH field. Cards are content-first: subject (course, discussion section, path, bookmark, followed Update) at the top, then a left thread line from that title through the actor avatar to the name (and through parent quote → reply name when someone replied). The final reply body has no line beside it. Follow tags on names are muted. Profile Updates in the feed can expand a discussion-style reply thread (click outside closes the reply UI). Requests to join your private learning paths appear in the Feed (and as a banner at the top of your profile) with Invite / Dismiss. They are not counted in the header unread-replies badge.

## Profile Updates

`/profile` and `/profile/{userId}` include an **Updates** tab for short posts (`profile_updates`, migration `050`). Own profile can compose; anyone can like and reply via polymorphic comments (`target_type = 'profile_update'`). Likes use `votes` with the same target type.
