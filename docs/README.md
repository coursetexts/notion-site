...
| [Community / research learning paths](./learning-paths.md) | Goal-based paths at `/learning-path/{slug}`. Private → public/collab needs 2 resources + a real why on every topic. Visitors see public vs collab on the date; outline edits are owner-only. **Export Context** copies an LLM prompt of the current step and outline. |
