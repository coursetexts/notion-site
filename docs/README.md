...
| [Knowledge](./knowledge.md)                                | `/paths/knowledge-graph` snapshot, profile Concepts tab (list), finish celebration, duration + enjoyment %, shared catalog + path occurrences. Profile graph is hidden. Daily LLM cron exists but is **disabled**. Partial/unwired features: [gaps.md](./gaps.md). |
