...
| --------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `community` (default) | Shared `LearningPath` shell · kicker “Learning Path” · community outline | Home / `/learning-paths` / `/all-courses` learning-paths view / profile → “create your own” | Home catalog, community lists, `/all-courses?view=learning-paths`, profile **Goals-based** filter                                                                                                                 |
| `research`            | Same shell · kicker “Research Learning Path” · community outline         | Field Atlas question → `/learning-path/new?kind=research`                                   | Profile **Research** / Goals-based filters, header pin empty-state, `/all-courses?view=learning-paths` / `?view=research`                                                                                                                        |
| `course`              | Same shell · kicker “Course Learning Path” · syllabus outline            | Migrated / seeded degree syllabus                                                           | Degrees, `/all-courses` **Academic** view (filled syllabi only), course pins, profile **Academic** filter — **not** the home community grid, the all-courses Goal-based view, or the profile Goals-based / Research filters |
...
| `/all-courses?view=learning-paths` | Public **community + research** cards (`listNonCourseLearningPaths`). Visible **All \| Goal-based \| Academic \| Research** filter vs unified Discover (`?view=all`). Always ends with **Can't find what you're looking for? Create your own path →** at the bottom of the list. |
