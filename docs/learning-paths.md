...
Goal-based maps at `/paths/learning-path/{slug}` (legacy `/learning-path/{slug}` redirects). Degree syllabi use the **same route** with `kind = course` (see [curated-courses.md](./curated-courses.md)).

## What they are

A path starts from a **goal** (“play a song on guitar”), then a graph of topics, prerequisites, and milestones, each with resources and notes. People can keep a path private, publish it, or open it for collaboration.

Three kinds (`learning_paths.kind`):

| Kind                  | UI                                                                       | How it is created                                                                                             | Where it shows                                                                                                                                                                                                                                     |
| --------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `community` (default) | Shared `LearningPath` shell · kicker “Learning Path” · community outline | Paths home / `/paths/learning-paths` / `/paths/all-courses` learning-paths view / profile → “create your own” | Paths home catalog, community lists, `/paths/all-courses?view=learning-paths`, profile **Goals-based** filter                                                                                                                                      |
| `research`            | Same shell · kicker “Research Learning Path” · community outline         | Field Atlas question → `/paths/learning-path/new?kind=research`                                               | Profile **Research** / Goals-based filters, header pin empty-state, `/paths/all-courses?view=learning-paths` / `?view=research`                                                                                                                    |
| `course`              | Same shell · kicker “Course Learning Path” · syllabus outline            | Migrated / seeded degree syllabus                                                                             | Degrees, `/paths/all-courses` **Academic** view (filled syllabi only), course pins, profile **Academic** filter — **not** the Coursetexts home open-courses grid, the Paths Goal-based catalog view, or the profile Goals-based / Research filters |

Official professor courses from Notion are **not** a `kind` yet. They stay at `/course/{pageId}` (and the Coursetexts `/all-courses` catalog). A later migrate will move those onto `learning_paths` as well; do not do that until that pass is designed. See [architecture — Future](./architecture.md#future-official-notion-courses).

The signed-in header pin menu lists saved courses and learning paths. Hovering a row (or focusing it) expands **N of M topics explored** for paths and syllabi, or **topics** for official Notion courses, plus **Next: {topic}** and **Continue →**. Continue opens the first unexplored node (`?node=`) or TOC section (`?topic=`). Official saved courses load heading labels from the course page when local TOC is missing (`POST /api/course-toc`). On touch devices the resume panel stays open because hover is unreliable.

## Routes

| Route                                    | UI                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/paths/learning-paths`                  | Catalog + “your paths”, search, create modal                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `/paths/all-courses?view=learning-paths` | Public **community + research** cards (`listNonCourseLearningPaths`). Visible **All \| Goal-based \| Academic \| Research** filter vs unified Discover (`?view=all`). Always ends with **Can't find what you're looking for? Create your own path →** at the bottom of the list. Discover / Goal-based search also merges semantic neighbors for learning paths (and Notion courses on Discover); see [semantic catalog search](./plans/semantic-learning-path-search/README.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `/paths/learning-path/new?goal=…`        | **Creation mode on the real `LearningPath` UI**. Hero kicker **New Learning Path**; outline **BUILD THE PATH** (no search). Sticky bottom bar: intro copy + **Fill out this path for me**, **Cancel learning path creation**, **Create private learning path**. No By You / Save row, resources, Discussions, notes, Context, or Explored until created. AI fill writes steps, topics, whys, and optional **Prerequisites** bullets (background not already covered earlier on the path).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `/paths/learning-path/{slug}`            | Shared learning-path shell. `kind` only changes the title kicker and the left outline. The hero byline is **By {owner}** and, when invites exist, **Collaborators:** with blue profile links. The date line is **publisher photo · Public** (or **Open to suggestions** / **Private**) **· Mon YYYY**, then a Save/••• chip (official-course Save style, bookmark icon). **•••** opens **Share** (copies the link), **Report** (omitted on your own path); owners also change visibility there. On any owned path the owner can **Invite editors** (Coursetexts email) to co-edit (no invitation email is sent). The owner and those invitees can edit titles and whys inline, and use **Edit path** to add or delete nodes. Invitees’ own official resources show **Added by you** (`addedByUserId` on the resource JSON). Owners (and resource authors) can open **Edit resource** to change or delete a resource (in-app confirm). A private slug you cannot read no longer renders an empty Coursetexts path: it shows **this learning path is private**. Signed-in visitors can **Request to join** (email stored on `learning_path_join_requests`; owner sees a banner on the path and on `/paths/profile`). An unknown slug shows **this learning path doesn’t exist yet** with a create CTA. On **Collab**, visitors suggest resources (dotted card, **Resource suggestion added by you**); the owner **Accept**s them onto the official list (owners are notified of new suggestions; suggesters of acceptance). **Context** (next to Discussions / Your Notes) opens a dialog: what to paste into an LLM, plus the copied prompt (current step and ancestors, numbered outline with whys, goal/summary). |

Canonical helpers: `lib/paths-routes.ts`. Bare `/learning-path/*`, `/learning-paths`, `/profile`, `/community`, `/degrees`, etc. permanently redirect into `/paths/…`.

The Paths home (`/paths`) shows “Try learning paths built by our community” with primary filters **Goal-based**, **Academic**, and **Research**. Selecting one reveals its sub-filters (Languages / Coding / Creative / Making for goal-based; Science / Math / Sociology / English for academic and research) and a single card grid scoped to that kind. Each card shows a small **Goal-based**, **Academic course**, or **Research-based** label above the title. Empty course placeholders are excluded. Under **A community for self-learners.**, **Learning Paths Tutorial** opens an in-page YouTube modal (`LearningPathsTutorialButton`). The **What is a learning path?** diagram is a looping visual (`HomeLearningPathDiagram`) of goal → topics → stacked resources → numbered list → notes → **Commit** → **Josh · Committed** with **Every day · 7:00 PM** (no reminder form); resource chips have a faint type icon on the right (video, paper, exercise, book). It does not create a path.

The Coursetexts home (`/`) instead highlights **Try open courses from top schools** (official Notion courses only) and links to `/all-courses`.

The profile **Paths** tab filters are **By you**, **Committed**, **Goals-based**, **Academic**, and **Research** on your own profile. On `/paths/profile/{userId}`, filters are **Committed**, **Goals-based**, **Academic**, and **Research** (read-only badges; no Commit / Notify / Continue). **Goals-based** = `kind=community`. **Academic** = official Notion course bookmarks plus `kind=course` syllabi (`isCourseKindPath`). **Research** = `kind=research`. **Committed** is a per-user flag (click Commit; hover Committed → Uncommit) in `learning_path_commitments`; committed cards sort to the top of the Paths list. You can commit without a reminder. After you commit, a **Notify** tag appears on card hover to the right of Commit; saving a cadence (every day / weekday + local time) stores `reminder_frequency`, `reminder_minute`, and `reminder_timezone` on that row. You cannot set a reminder without committing. Sending those notifications is not built yet. Existing DBs: apply `041_learning_path_commitment_reminders.sql` (creates the commitments table if `030` was never applied). Public profiles reading commitments need `052_public_learning_path_commitments_read.sql`. Unwired Paths-tab features are listed in [gaps.md](./gaps.md). Community/research/course-kind cards show muted byline text under the title (**Created by you · Private**, or **By {name} · Public** / Open to suggestions). Paths you were invited to are not “Created by you”. Saved official Notion courses and pinned Coursetexts syllabi also get a byline (**By {professors} · {school} · Public** when the course page has been opened, otherwise **By {school} · Public** or **By Coursetexts · Public**). Cards with marked-complete sections or explored topics show a muted **97% complete** (etc.) tag to the left of Commit (hidden at 0%).

Profile tabs also include **Knowledge** (acquired topics) and **Notes** (your private topic notes from courses and learning paths; `/profile` only; editable, with Open jumping to that topic and the notes panel). Finishing a path records unique node labels, shows blue confetti and a topics modal, and adds a **What you learned** outline row. The bottom **StepNavBar** stays pinned under a scrollable path pane (same shell as the sticky Discussions / Your Notes row) and shows **Mark as explored** (dark primary) and quiet **✓ Explored** after; click it to mark unexplored. Completing a topic (or the whole path) asks how long it took and how enjoyable learning was with the given resources (0–100%). The left outline shows the topic name and a light-blue stroke check when `explored` (parents and children). It does not print Exploring / Need this / As deep as you need. On a topic, the **title**, **Why is this on the learning path:**, and optional **Prerequisites:** edit inline (hover pencil). **Prerequisites** sits under Why and above **Resources** (hidden when empty for readers; owners/invitees can always add or edit; after AI fill on `/new`, the empty state shows too). Outline **Edit path** inserts an inline blue title field under or after the topic (Enter to save, Esc to cancel), or deletes the current node. **Resources** start open. **Overview** has Resources only — no Why section (the path summary stays in the hero). On Overview, **Commit & Remind Me** sits on the bottom step bar next to **Start learning path** (opens the first outline step; syllabi with no topics yet jump to Resources). Official Notion courses use the same control on the **General** tab (`course:{notion_page_id}`). Official Notion courses, community paths, and course syllabi share that bar: **← Previous** (hidden on the first step), **N of M**, explored, and **Next →** (dark primary; **Finish path** on the last step). Below the path content, **Discuss this with others?** sits beside **People on this learning path** (study-circle members from `path.circle.members`). See [knowledge.md](./knowledge.md). Daily Gemini linking of the shared catalog is **implemented but disabled**.
...
```text
{
  ...
  nodes[]   id, label, kind: goal | concept | prerequisite | milestone
            ...
            prerequisites?  optional string[] — layman background not
            already covered earlier on the path (shown under Why, before
            Resources)
