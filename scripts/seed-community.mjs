 ...
 * comment thread — for local / disposable-project testing.
 *
 * Targets the legacy DB after migration 025_community_page_platform.sql:
 *   profiles(user_id = auth uid, karma_score), resources, comments, votes.
 *
...
async function main() {
  // 1. Users + profiles. Legacy schema: profiles.user_id = auth uid.
  //    Signup trigger creates the row; we then set display name + karma.
  const users = []
  ...
  // 5. Votes: resource votes + comment votes.
  {
