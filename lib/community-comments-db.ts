/**
 * Community platform data layer — targets the LIVE schema
 * (000_community_platform_init): resources, comments, votes, profiles.
 *
 * Conventions there: profiles.id IS the auth user id; comments and votes are
 * polymorphic via (target_type, target_id); comments thread through
 * parent_comment_id; profiles.karma_score is display-only (rules TBD,
 * see lib/karma.ts).
 */
...
async function getAuthorsByIds(
  ...
  const { data } = await supabase
    .from('profiles')
    .select('id, display_name, avatar_url, karma_score')
    .in('id', userIds)
