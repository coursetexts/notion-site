/**
 * Load Notion university courses shown on the home page (same source as
 * pages/index getStaticProps). Safe for Node scripts — no React / CSS / pages.
 */
...
export type NotionHomeCourseCard = {
  id: string
  href: string
  meta: string
  title: string
...
export function fallbackNotionHomeCourses(): NotionHomeCourseCard[] {
  const fallbackMeta = [
 ...
 */
export function extractHomeCoursesFromRootPage(params: {
  pageMap: Record<string, ExtendedRecordMap>
  canonicalPageMap: Record<string, string>
  rootNotionPageId: string
}): {
  courses: NotionHomeCourseCard[]
  ...
  const out: NotionHomeCourseCard[] = []
  let i = 0
...
/**
 * Same course pool as home getStaticProps (root listing, then canonicalPageMap
 * walk, then static fallback). Does not load academic/curated course paths.
 */
export async function loadNotionHomePageCourses(): Promise<{
  courses: NotionHomeCourseCard[]
  notionHomeDebug: NotionHomeDebugPayload | null
}> {
  ...
  const courses = pool.length > 0 ? pool : fallbackNotionHomeCourses()
  ...
  return { courses, notionHomeDebug }
}

export async function loadNotionHomeCourses(): Promise<NotionHomeCourseCard[]> {
  const { courses } = await loadNotionHomePageCourses()
  return courses
}
