 ...
 * Karma updates.
 *
 * `profiles.karma_score` is displayed next to usernames (migration
 * 025_community_page_platform), but HOW votes translate into karma is not
 * decided yet, so this module is deliberately a no-op. Call sites are already
 * wired (comment/vote flows) so the rules can land here without touching the UI.
 */
