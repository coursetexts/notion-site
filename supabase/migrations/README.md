...
| `042_learning_path_invites.sql` | Collaborator invites by email. Invitee must already have a `profiles` row. No invitation email. Invitees can read and edit outline `data` (private originally; widened in `053`) |
...
| `051_profile_update_reposts.sql` | Repost (`repost_of_id`) and quote (`quote_of_id`) columns on `profile_updates`. |
...
Existing projects that already ran through `037` should apply `038` (do not re-run `000`). If `038` already ran with a 1–5 rating check, apply `039`. Apply `040` so collaborative paths cannot rewrite the outline. Apply `041` for Learn-tab commitments + reminder cadence (`041` creates the table if `030` was never applied). Apply `042` for collaborator invites (originally private-only). Apply `043` so private/unknown path URLs show an access/missing state. Apply `044` so signed-in visitors can request to join a private path. Apply `045` for `/knowledge-graph` topic–path occurrences. If you applied the removed `046_official_course_content`, run `047_revert_official_course_content.sql`. Run `048_drop_community_wall_and_notebooks.sql` to remove legacy wall + notebook tables. Apply `049_profile_learning_summary.sql` for public bio and learning summary on profiles. Apply `050_profile_updates.sql` for profile Updates posts, likes, and comments. Apply `051_profile_update_reposts.sql` for repost and quote. Apply `052_public_learning_path_commitments_read.sql` so public profiles can list another user’s committed Learning items. Apply `053_learning_path_invites_any_visibility.sql` so owners can invite collaborators on public and collaborative paths too. Apply `054_learning_path_invites_readable_on_path.sql` so the people sidebar can list collaborators for path readers.
...
- [ ] Owned path (Private / Public / Collab): owner **Invite** by email of someone already on Coursetexts; invitee signed in with that email can open and edit the outline; owner can Remove. Invitee-added official resources show **Added by you**. Apply `042` + `053_learning_path_invites_any_visibility.sql`. No invitation email is sent.
...
- [ ] Collaborative path: visitor does **not** see **Edit path**; owner/invitees still do. Visitor **Suggest a resource** shows a dotted card; owner **Accept** copies it into the official list. Apply `040_learning_path_outline_owner_only.sql` (outline) and `032`/`034` (suggestions).
- [ ] People sidebar: invited collaborators first with `(collaborator)`, then public savers. Apply `054_learning_path_invites_readable_on_path.sql`.
- [ ] Topic title and why edit inline (hover pencil). **Edit path** menu: add under, add after, delete current node.
